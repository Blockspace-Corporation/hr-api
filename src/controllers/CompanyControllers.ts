...
export const readAllCompany = async (
  ...
  export const readOneCompanyHandler = async (
    request: FastifyRequest,
    reply: FastifyReply
  ) => {
    try {
      ...
      const targetUser = await CompanyRepository.getCompanyById(requestParams.id);
      ...
      console.error('readOneCompanyHandler: error trying to read user: ${error}');
      reply.internalServerError(String(error || 'Unknown error occurred.'));
    }
  };